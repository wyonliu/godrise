...
### 第一部：灵机江湖
- 核心主题：觉醒——什么是真实？灵机、武侠、东方神话的融合
- 预计字数：40万字（卷一25章/17.4万字已完稿）
- 时间跨度：2026-2040年
...
- **Cloudflare Pages**：静态资源部署（自动从 GitHub 部署）
- **Cloudflare Workers**：评论/投票 API（`functions/api/`）
- **Cloudflare KV**：评论与投票持久化存储
...
- 章节评论 + 划线批注 + 章节投票
- 暗色/亮色主题
- 手机端滑动翻页 + safe-area 适配
- URL hash 路由（支持分享链接和前进/后退）
