...
def phase_images(panels, engine_name, out_dir, start, end):
    ...
    for i, panel in enumerate(panels):
        ...
        if not prompt:
            print(f"  [Panel {panel_num:03d}/{total}] 无 image_prompt，跳过")
            continue
...
def phase_voice(panels, out_dir, start, end):
    ...
    for i, panel in enumerate(panels):
        ...
        speaker = panel.get("speaker", "旁白")
        speed = panel.get("speed", 1.0)

        if not dialogue:
            print(f"  [Panel {panel_num:03d}/{total}] 无对白，跳过")
            continue

        out_path = audio_dir / f"panel_{panel_num:03d}.mp3"
        ...
        # 清理舞台提示
        clean_text = STAGE_DIR_RE.sub("", dialogue).strip()
        if not clean_text:
            continue
        ...
        label = speaker
        print(f"  [Panel {panel_num:03d}/{total}] {label} x{actual_speed}  {clean_text[:40]}...")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for idx, item in enumerate(available):
            ...
            if audio_path:
                ...
                # 至少留 0.5s 呼吸
                dur_s = max(dur_s + 0.3, panel.get("duration", 4.0))
            else:
                dur_s = panel.get("duration", 4.0)

            effect = panel.get("camera", "zoom_in")
            dialogue = panel.get("dialogue", "")

            print(f"  [Panel {pnum:03d}] {dur_s:.1f}s {effect} {dialogue[:30]}...")
            ...
            # 2. 叠加字幕
            if dialogue:
                sub_img = make_subtitle_image(dialogue, W, H, font_size=42)
            ...
            try:
                ...
                for ap in audio_parts:
                    ...
                    if panel_idx < len(panels):
                        speaker = panels[panel_idx].get("speaker", "")
                        if speaker == "麦洛" and MELODY_VOL_DB != 0:
...
def main():
    ...
    total_chars = sum(len(p.get("dialogue", "")) for p in panels[start-1:end])
